package com.networkgraph.entity;
...
public class NetworkGraphData {
    ...
    */
    ...
    public static FromToModel getModel(int[] numbers) {
        FromToModel model = new DefaultFromToModel();

        // Generate the graph based on the input array
        for (int i = 0; i < numbers.length - 1; i++) {
            for (int j = 0; j < numbers[i]; j++) {
                for (int k = 0; k < numbers[i + 1]; k++) {
                    model.addValue("number-tree", " " + (i + 1) + "-" + (j + 1), " " + (i + 2) + "-" + (k + 1));
                }
            }
        }
        return model;
